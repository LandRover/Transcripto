...
def tts_gpt_openai(text, tts_model="gpt-4o-audio-preview", voice="alloy", format="mp3", force=False):
    try:
        client = OpenAI(
...
def tts_tts1_openai(text, tts_model="tts-1", voice="alloy", format="mp3", force=False):
    try:
        client = OpenAI(
            api_key = os.environ.get("OPENAI_API_KEY"),
        )
